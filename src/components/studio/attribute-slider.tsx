...
export function AttributeSlider({ attribute, onChange }: AttributeSliderProps) {
  return (
    <div className="flex items-center gap-3">
      <label className="min-w-[60px] text-[11px] text-[var(--angel-text-soft)] shrink-0">
        {attribute.name}
      </label>
      <input
        type="range"
      ...
      />
      <span className="min-w-[28px] text-right text-[10px] font-medium text-[var(--angel-text-soft)]">
        {attribute.value}
      </span>
    </div>
  );
