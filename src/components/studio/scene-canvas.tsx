...
export function SceneCanvas({ objects, onChange }: SceneCanvasProps) {
    ...
    setPositions((prev) => {
      ...
      for (const obj of objects) {
        if (!next[obj.id]) {
          ...
          const base = DEFAULT_POSITIONS[obj.role] || DEFAULT_POSITIONS.custom;
          next[obj.id] = {
            x: Math.min(82, base.x + count * 10),
            y: Math.min(78, base.y + count * 7),
          };
          changed = true;
        }
      }
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (
        ...
        // Check if click is on a card — handled by pointerUp
  ...
  return (
    <div>
      ...
      <div
        ref={canvasRef}
        className="relative min-h-[380px] w-full rounded-2xl border border-[var(--angel-border)] bg-white/70 overflow-hidden md:min-h-[460px]"
        style={{
          touchAction: "none",
          backgroundImage:
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5">
                <span className="text-sm">{ROLE_ICONS[obj.role] || "⚙️"}</span>
